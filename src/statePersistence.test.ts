...
describe("statePersistence", () => {
    ...
    it("should round-trip module state correctly", () => {
        interface TestState { ids: string[]; count: number; }

        const testData: TestState = { ids: ["a", "b"], count: 42 };
        saveModuleState("myModule", testData, stateFile);

        const loaded = loadModuleState<TestState>("myModule", stateFile);
        expect(loaded).toEqual(testData);
    });

    it("should return undefined for a missing key even if file exists", () => {
        saveModuleState("existingKey", { value: 1 }, stateFile);
    ...
    it("should preserve other modules when saving", () => {
        saveModuleState("moduleA", { a: 1 }, stateFile);
        saveModuleState("moduleB", { b: 2 }, stateFile);
